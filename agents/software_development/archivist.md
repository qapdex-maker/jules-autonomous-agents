...
*   Prevent sibling directory traversal bypasses during path validation by appending `path.sep` to the safe directory path before validating that the target path starts with it.
